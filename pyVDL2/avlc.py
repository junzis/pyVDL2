...
class AVLC(object):
    """AVLC is the protocal for the data link in VDL2 communication."""

    def __init__(self):
        super(AVLC, self).__init__()

    def decode(self, hexframe):
        """Deocde information from the ALVC frame.

        Args:
            hexframe (String): hexadecimal string

        Returns:
            dict: Information contained in the AVLC frame

        """
        if len(hexframe) < 18:
            return None

        addr = self.addresses(hexframe[:16])
        link_control = self.linkcontrol(hexframe[16:18])
    ...
    def addresses(self, hexaddr):
        ...
        """
        source = self._decode_single_address(hexaddr[8:])
        destination = self._decode_single_address(hexaddr[:8])

        return {"source": source, "destination": destination}

    def linkcontrol(self, hexlc):
    ...
    def _decode_single_address(self, hex8):
